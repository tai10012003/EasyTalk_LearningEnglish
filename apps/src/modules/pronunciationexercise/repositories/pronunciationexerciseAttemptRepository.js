...
class PronunciationExerciseAttemptRepository {
    constructor() {
        this.client = DatabaseConnection.getMongoClient();
        this.db = this.client.db(config.mongodb.database);
        this.collection = this.db.collection("pronunciationexerciseattempts");
        this.ensureIndexes();
    }

    ensureIndexes() {
        this.collection.createIndex({ userId: 1, pronunciationExerciseId: 1, createdAt: -1 }).catch(() => {});
    ...
    async deleteById(id, userId) {
        if (!ObjectId.isValid(id) || !ObjectId.isValid(userId)) return { deletedCount: 0 };
...
module.exports = PronunciationExerciseAttemptRepository;
