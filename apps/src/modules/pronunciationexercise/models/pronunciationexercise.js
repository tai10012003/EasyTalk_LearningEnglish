...
class PronunciationExercise {
    constructor({ _id = null, title, questions = [], slug = "", sort = 0, display = true, createdAt = new Date(), updatedAt = null }) {
        this._id = _id;
        this.title = title;
        this.questions = questions.map(q => new PronunciationQuestion(q));
        this.slug = slug;
        this.sort = sort;
    ...
    addQuestion(question, type, correctAnswer, explanation, options = []) {
        const q = new PronunciationQuestion({ question, type, correctAnswer, explanation, options });
    ...
    static validate(doc) {
        ...
        if(doc.questions && Array.isArray(doc.questions)) {
            doc.questions.forEach((q, index) => {
                const qErrors = PronunciationQuestion.validate(q);
                if(qErrors.length > 0) {
...
module.exports = { PronunciationExercise, PronunciationQuestion };