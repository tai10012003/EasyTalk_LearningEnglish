...
const { validatePronunciationInput, parseQuizzes } = require("../validators/pronunciationValidator");
    ...
    try {
        ...
        if (!isUnlocked) {
            return res.status(403).json({ 
                success: false, 
                message: "This pronunciation is locked for you. Please complete previous pronunciations first." 
            });
        }
...
router.post("/api/pronunciation/complete/:id", verifyToken, async (req, res) => {
    try {
        ...
        const pronunciation = await pronunciationService.getPronunciation(pronunciationId);
        if (!pronunciation) {
            return res.status(404).json({ success: false, message: "Pronunciation not found" });
        }
        let userProgress = await userProgressService.getUserProgressByUserId(userId);
        if (!userProgress) {
            const firstPage = await pronunciationService.getPronunciationList(1, 1);
        ...
        const isUnlocked = (userProgress.unlockedPronunciations || []).some(s => s.toString() == pronunciationId.toString());
        if (!isUnlocked) {
            return res.status(403).json({ success: false, message: "You cannot complete a locked pronunciation." });
        }
        const all = await pronunciationService.getPronunciationList(1, 10000);
        const allPronunciations = all?.pronunciations || [];
        const idx = allPronunciations.findIndex(s => s._id.toString() == pronunciationId.toString());
        ...
        return res.json({
            ...
            userProgress: {
                unlockedYs: updatedUserProgress.unlockedPronunciations,
