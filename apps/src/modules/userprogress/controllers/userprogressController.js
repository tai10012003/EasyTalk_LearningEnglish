...
function createUserProgressController({
    userProgressService,
    badgeService,
    userPrizeService,
    leaderboardService,
    ...
    if (missing.length) {
        throw new Error(`createUserProgressController requires: ${missing.join(', ')}`);
    }
    ...
    router.post("/update-goal", verifyToken, asyncHandler(async (req, res) => {
        const userId = req.user.id;
        const { goal } = req.body;
        await badgeService.updateDailyFlashcardGoal(userId, goal);
...
module.exports = { createUserProgressController };
