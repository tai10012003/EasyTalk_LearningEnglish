...
const UpdateReminder = ({ isOpen, onClose, onUpdated, reminder }) => {
    ...
    useEffect(() => {
        if (isOpen && reminder) {
            ...
            setFrequency(reminder.frequency || "one-time");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await ReminderService.updateReminder(reminder._id, payload);
            await Swal.fire({ icon: "success", title: t("reminderPage.alert.successTitle"), text: t("reminderPage.alert.updateSuccess") });
            onUpdated && onUpdated();
            onClose && onClose();
        } catch (err) {
            console.error("Update reminder error:", err);
            Swal.fire({ icon: "error", title: t("reminderPage.alert.errorTitle"), text: err.message || t("reminderPage.alert.updateFailed") });
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="custom-modal-overlay" onClick={onClose}>
            <div className="custom-modal" onClick={(e) => e.stopPropagation()}>
                <div className="custom-modal-header">
                    <h5>{t("reminderPage.form.updateTitle")}</h5>
