...
function TranslatePronunciation() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const [pronunciationData, translation] = await Promise.all([
                    PronunciationService.getPronunciation(id),
                    EnglishTranslationService.getTranslation("pronunciation", id)
                ]);
                ...
                console.error("Error loading pronunciation translation:", error);
                Swal.fire("Thất bại!", "Không thể tải dữ liệu dịch bài học phát âm.", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    const originalQuizzes = useMemo(() => formatJson(pronunciation?.quizzes), [pronunciation]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await EnglishTranslationService.saveTranslation("pronunciation", id, {
                sourceSlug: pronunciation?.slug || "",
                fields: payloadFields
            });
            Swal.fire("Thành công!", "Bản dịch tiếng Anh đã được lưu.", "success");
        } catch (error) {
            console.error("Error saving pronunciation translation:", error);
    ...
    const handleDelete = async () => {
        const result = await Swal.fire({
            title: "Xóa bản dịch?",
            text: "Bản dịch tiếng Anh của bài học phát âm này sẽ bị xóa.",
            icon: "warning",
        ...
        try {
            await EnglishTranslationService.deleteTranslation("pronunciation", id);
            setFields(emptyFields);
            Swal.fire("Thành công!", "Đã xóa bản dịch tiếng Anh.", "success");
        } catch (error) {
            console.error("Error deleting pronunciation translation:", error);
            Swal.fire("Thất bại!", "Không thể xóa bản dịch.", "error");
        }
    };

    if (loading) return <p>Đang tải dữ liệu...</p>;
    if (!pronunciation) return <p>Không tìm thấy bài học phát âm.</p>;

    return (
        <div className="admin-translation-page">
            <div className="admin-translation-header">
                <div>
                    <h1>DỊCH TIẾNG ANH BÀI HỌC PHÁT ÂM</h1>
                    <p>Quản lý bản dịch riêng trong collection englishtranslations.</p>
                </div>
                <a href="/admin/pronunciation" className="admin-translation-back">Quay lại</a>
            </div>
            ...
            <div className="admin-translation-grid">
                <section className="admin-translation-panel">
                    ...
                    <label>Tiêu đề</label>
                    <div className="admin-translation-preview">{pronunciation.title}</div>
                    <label>Mô tả</label>
                    <div className="admin-translation-preview">{pronunciation.description}</div>
                    <label>Danh mục</label>
                    <div className="admin-translation-preview">{pronunciation.category}</div>
                    <label>Nội dung</label>
                    <div className="admin-translation-preview multiline">{pronunciation.content}</div>
                    <label>Quizzes</label>
                ...
                <section className="admin-translation-panel">
                    ...
                    <input
                        ...
                        placeholder="Short Vowels and the IPA Chart"
                    />
                    ...
                    <input
                        ...
                        placeholder="Module 1: Alphabet and Basic Sounds"
                    />
                    ...
                    <textarea
                        ...
                        rows={10}
                        placeholder="English pronunciation lesson content..."
                    />
...
export default TranslatePronunciation;
