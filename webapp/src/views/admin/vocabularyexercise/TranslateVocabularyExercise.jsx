...
function TranslateVocabularyExercise() {
    const { id } = useParams();
        ...
        const fetchData = async () => {
            ...
            try {
                const [exerciseData, translation] = await Promise.all([
                    VocabularyExerciseService.getVocabularyExerciseAdmin(id),
                    EnglishTranslationService.getTranslation("vocabularyExercise", id)
                ]);
                const originalExercise = exerciseData?.data || exerciseData;
                const translationFields = translation?.fields || {};
                setVocabularyExercise(originalExercise);
                setFields({
                    title: translationFields.title || "",
                    questions: translationFields.questions ? formatJson(translationFields.questions) : ""
                });
            } catch (error) {
                console.error("Error loading vocabulary exercise translation:", error);
                Swal.fire("Thất bại!", "Không thể tải dữ liệu dịch bài luyện tập từ vựng.", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    const originalQuestions = useMemo(() => formatJson(vocabularyExercise?.questions), [vocabularyExercise]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await EnglishTranslationService.saveTranslation("vocabularyExercise", id, {
                sourceSlug: vocabularyExercise?.slug || "",
                fields: payloadFields
            });
            Swal.fire("Thành công!", "Bản dịch tiếng Anh đã được lưu.", "success");
        } catch (error) {
            console.error("Error saving vocabulary exercise translation:", error);
    ...
    const handleDelete = async () => {
        ...
        try {
            await EnglishTranslationService.deleteTranslation("vocabularyExercise", id);
            setFields(emptyFields);
            Swal.fire("Thành công!", "Đã xóa bản dịch tiếng Anh.", "success");
        } catch (error) {
            console.error("Error deleting vocabulary exercise translation:", error);
            Swal.fire("Thất bại!", "Không thể xóa bản dịch.", "error");
        }
    };

    if (loading) return <p>Đang tải dữ liệu...</p>;
    if (!vocabularyExercise) return <p>Không tìm thấy bài luyện tập từ vựng.</p>;

    return (
        <div className="admin-translation-page">
            <div className="admin-translation-header">
                <div>
                    <h1>DỊCH TIẾNG ANH BÀI LUYỆN TẬP TỪ VỰNG</h1>
                    <p>Quản lý bản dịch riêng trong collection englishtranslations.</p>
                </div>
                <a href="/admin/vocabulary-exercise" className="admin-translation-back">Quay lại</a>
            </div>
            ...
            <div className="admin-translation-grid">
                <section className="admin-translation-panel">
                    ...
                    <label>Tiêu đề</label>
                    <div className="admin-translation-preview">{vocabularyExercise.title}</div>
                ...
                <section className="admin-translation-panel">
                    ...
                    <input
                        ...
                        placeholder="Vocabulary Exercise Topic 1: Family"
                    />
...
export default TranslateVocabularyExercise;
