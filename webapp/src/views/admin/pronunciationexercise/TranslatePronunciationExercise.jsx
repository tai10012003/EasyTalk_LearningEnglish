...
function TranslatePronunciationExercise() {
    const { id } = useParams();
        ...
        const fetchData = async () => {
            ...
            try {
                const [exerciseData, translation] = await Promise.all([
                    PronunciationExerciseService.getPronunciationExerciseAdmin(id),
                    EnglishTranslationService.getTranslation("pronunciationExercise", id)
                ]);
                const originalExercise = exerciseData?.data || exerciseData;
                const translationFields = translation?.fields || {};
                setPronunciationExercise(originalExercise);
                setFields({
                    title: translationFields.title || "",
                    questions: translationFields.questions ? formatJson(translationFields.questions) : ""
                });
            } catch (error) {
                console.error("Error loading pronunciation exercise translation:", error);
                Swal.fire("Thất bại!", "Không thể tải dữ liệu dịch bài luyện tập phát âm.", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    const originalQuestions = useMemo(() => formatJson(pronunciationExercise?.questions), [pronunciationExercise]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await EnglishTranslationService.saveTranslation("pronunciationExercise", id, {
                sourceSlug: pronunciationExercise?.slug || "",
                fields: payloadFields
            });
            Swal.fire("Thành công!", "Bản dịch tiếng Anh đã được lưu.", "success");
        } catch (error) {
            console.error("Error saving pronunciation exercise translation:", error);
    ...
    const handleDelete = async () => {
        ...
        try {
            await EnglishTranslationService.deleteTranslation("pronunciationExercise", id);
            setFields(emptyFields);
            Swal.fire("Thành công!", "Đã xóa bản dịch tiếng Anh.", "success");
        } catch (error) {
            console.error("Error deleting pronunciation exercise translation:", error);
            Swal.fire("Thất bại!", "Không thể xóa bản dịch.", "error");
        }
    };

    if (loading) return <p>Đang tải dữ liệu...</p>;
    if (!pronunciationExercise) return <p>Không tìm thấy bài luyện tập phát âm.</p>;

    return (
        <div className="admin-translation-page">
            <div className="admin-translation-header">
                <div>
                    <h1>DỊCH TIẾNG ANH BÀI LUYỆN TẬP PHÁT ÂM</h1>
                    <p>Quản lý bản dịch riêng trong collection englishtranslations.</p>
                </div>
                <a href="/admin/pronunciation-exercise" className="admin-translation-back">Quay lại</a>
            </div>
            ...
            <div className="admin-translation-grid">
                <section className="admin-translation-panel">
                    ...
                    <label>Tiêu đề</label>
                    <div className="admin-translation-preview">{pronunciationExercise.title}</div>
                ...
                <section className="admin-translation-panel">
                    ...
                    <input
                        ...
                        placeholder="Pronunciation Exercise Set 1"
                    />
...
export default TranslatePronunciationExercise;
